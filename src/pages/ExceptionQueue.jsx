  ...
  return (
    <div className="main-content">
      <div style={{ display: 'flex', gap: '24px' }}>
        <div style={{ flex: selectedEx ? '1' : '1.5', transition: 'all 0.3s ease' }}>
          ...
          <div className="card" style={{ padding: '0' }}>
            <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-light)', display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div style={{ fontSize: '13px', color: '#64748B', fontWeight: '500' }}>Showing {exceptions.length} exceptions</div>
            </div>
        ...
        {selectedEx && (
          <div className="card animate-fade-in" style={{ flex: '0.8', padding: '0', height: 'fit-content', position: 'sticky', top: '88px' }}>
            <div style={{ padding: '24px', borderBottom: '1px solid var(--border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <Zap size={18} color="var(--gold)" />
                <h3 style={{ fontSize: '16px' }}>Match Suggestions</h3>
              </div>
              <button onClick={() => setSelectedEx(null)} className="btn-outline" style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}>✕</button>
            </div>

            <div style={{ padding: '24px' }}>
              <div style={{ background: '#F8FAFC', padding: '16px', borderRadius: '8px', border: '1px solid #E2E8F0', marginBottom: '24px' }}>
                <div style={{ fontSize: '12px', color: '#64748B', fontWeight: '600', textTransform: 'uppercase', marginBottom: '8px' }}>Source Exception</div>
                ...
                <div style={{ fontSize: '13px', color: '#64748B', marginTop: '4px' }}>Ref: {selectedEx.ref}</div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {suggestions.map((sug, i) => (
                  <div key={i} style={{ 
                    ...
                    <p style={{ fontSize: '13px', color: '#64748B', lineHeight: '1.5', marginBottom: '16px' }}>{sug.reason}</p>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button className="btn btn-primary" style={{ flex: 1, height: '32px', fontSize: '12px' }}>Accept Match</button>
                      <button className="btn btn-outline" style={{ flex: 1, height: '32px', fontSize: '12px' }}>Review Candidate</button>
                    </div>
                  </div>
                ))}
              </div>

              <button className="btn btn-outline" style={{ width: '100%', marginTop: '24px', borderColor: 'var(--primary)', color: 'var(--primary)' }}>
